...
{
    public partial class ReportTechnics_AVIATION_EQUIP : RESPage
    {
        ...
        public override string PageUIKey
        {
            get
            {
                return "RES_REPORTS_AVIATION_EQUIP";
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (GetUIItemAccessLevel("RES_REPORTS") == UIAccessLevel.Hidden ||
                GetUIItemAccessLevel("RES_REPORTS_AVIATION_EQUIP") == UIAccessLevel.Hidden)
            {
            ...
            ReportAviationEquipManageFilter filter = CollectFilterData();

            int allRows = ReportAviationEquipUtil.GetAllReportAviationEquipManageBlocksCount(filter, CurrentUser);
            ...
            if (!IsPostBack)
            {
                ...
                this.btnPrintReportAllAviationEquips.Visible = false;
        ...
        private void RefreshDataGrid()
        {
            ...
            List<ReportAviationEquipManageBlock> aviationEquipManageBlocks = ReportAviationEquipUtil.GetAllReportAviationEquipManageBlocks(filter, pageLength, CurrentUser);

            //No data found
            if (aviationEquipManageBlocks.Count == 0)
            {
            ...
            {
                this.btnPrintReportAllAviationEquips.Visible = true;
...
(!isNormativeTechnicsHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(10);'>Нормативна категория" + arrOrderCol[9] + @"</th>" : "") +
     (!isAirInvNumberHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(1);' title='Инвентарен номер'>Инв. номер" + arrOrderCol[0] + @"</th>" : "") +
 ...
 (!isAviationAirModelHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(5);'>Модел" + arrOrderCol[4] + @"</th>" : "") +
    (!isMilDepartmentHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(6);'>На отчет в" + arrOrderCol[5] + @"</th>" : "") +
     (!isMilRepStatusHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(7);'>Състояние по отчета" + arrOrderCol[6] + @"</th>" : "") +
        ...
        (!isAddressHidden   ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(9);'>Адрес" + arrOrderCol[8] + @"</th>" : "") + @"
                ...
                foreach (ReportAviationEquipManageBlock reportAviationEquipManageBlock in aviationEquipManageBlocks)
                {
                    string cellStyle = "vertical-align: top;";

                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"'>
                                 <td style='" + cellStyle + @"'>" + ((pageIdx - 1) * pageLength + counter).ToString() + @"</td>" +
  (!isNormativeTechnicsHidden ? "<td style='" + cellStyle + @"' title='" + reportAviationEquipManageBlock.NormativeTechnicsName + @"'>" + reportAviationEquipManageBlock.NormativeTechnicsCode + @"</td>" : "") +
       (!isAirInvNumberHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.AirInvNumber + @"</td>" : "") +
   (!isTechnicsCategoryHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.TechnicsCategory + @"</td>" : "") +
    (!isAviationAirKindHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.AviationAirKind + @"</td>" : "") +
    (!isAviationAirTypeHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.AviationAirType + @"</td>" : "") +
   (!isAviationAirModelHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.AviationAirModel + @"</td>" : "") +
      (!isMilDepartmentHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.MilitaryDepartment + @"</td>" : "") +
       (!isMilRepStatusHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.MilitaryReportStatus + @"</td>" : "") +
          (!isOwnershipHidden ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.Ownership + @"</td>" : "") +
          (!isAddressHidden   ? "<td style='" + cellStyle + @"'>" + reportAviationEquipManageBlock.Address + @"</td>" : "") + @"
        ...
        private ReportAviationEquipManageFilter CollectFilterData()
        {
            ...
            if (ddAviationAirKind.SelectedValue != ListItems.GetOptionAll().Value)
            {
                ...
                this.hdnAviationAirKindId.Value = ddAviationAirKind.SelectedValue;
            }
            else
            {
                this.hdnAviationAirKindId.Value = "";
            }
            ...
            if (ddAviationAirType.SelectedValue != ListItems.GetOptionAll().Value)
            {
                ...
                this.hdnAviationAirTypeId.Value = ddAviationAirType.SelectedValue;
            }
            else
            {
                this.hdnAviationAirTypeId.Value = "";
            }
            ...
            //    this.hdnAviationAirModelId.Value = ddAviationAirModel.SelectedValue;
            //}
            //else
            //{
            //    this.hdnAviationAirModelId.Value = "";
            //}

            this.hdnAviationAirModelName.Value = txtAviationAirModelName.Text;
            ...
            this.hdnInventoryNumber.Value = txtAirInvNumber.Text;
