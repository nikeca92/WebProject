...
{
    public partial class ReportTechnics_FUEL_CONTAINERS : RESPage
    {
        ...
        public override string PageUIKey
        {
            get
            {
                return "RES_REPORTS_FUEL_CONTAINERS";
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (GetUIItemAccessLevel("RES_REPORTS") == UIAccessLevel.Hidden ||
                GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS") == UIAccessLevel.Hidden)
            {
            ...
            ReportFuelContainerManageFilter filter = CollectFilterData();

            int allRows = ReportFuelContainerUtil.GetAllReportFuelContainerManageBlocksCount(filter, CurrentUser);
        ...
        private void RefreshDataGrid()
        {
            ...
            List<ReportFuelContainerManageBlock> reportFuelContainerManageBlocks = ReportFuelContainerUtil.GetAllReportFuelContainerManageBlocks(filter, pageLength, CurrentUser);

            //No data found
            if (reportFuelContainerManageBlocks.Count == 0)
            {
                html = "<span>Няма намерени резултати</span>";
                this.btnPrintReportAllFuelContainers.Visible = false;
                this.btnExport.Visible = false;
            }
            //If there is data then generate dynamically the HTML for the data grid
            else 
            {
                this.btnPrintReportAllFuelContainers.Visible = true;
...
(!isFuelContainerCountHidden ? "<th style='width: 60px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(5);'>Брой" + arrOrderCol[4] + @"</th>" : "") + 
    (!isMilDepartmentHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(6);'>На отчет в" + arrOrderCol[5] + @"</th>" : "") +
     (!isMilRepStatusHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(7);'>Състояние по отчета" + arrOrderCol[6] + @"</th>" : "") +
        ...
        (!isAddressHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(9);'>Адрес" + arrOrderCol[8] + @"</th>" : "") + @"
                ...
                foreach (ReportFuelContainerManageBlock fuelContainerManageBlock in reportFuelContainerManageBlocks)
                {
                    string cellStyle = "vertical-align: top;";

                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"'>
                                 <td style='" + cellStyle + @"'>" + ((pageIdx - 1) * pageLength + counter).ToString() + @"</td>" +
  (!isNormativeTechnicsHidden ? "<td style='" + cellStyle + @"' title='" + fuelContainerManageBlock.NormativeTechnicsName + @"'>" + fuelContainerManageBlock.NormativeTechnicsCode + @"</td>" : "") +
    (!isInventoryNumberHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.InventoryNumber + @"</td>" : "") +
   (!isTechnicsCategoryHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.TechnicsCategory + @"</td>" : "") +
  (!isFuelContainerKindHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.FuelContainerKind + @"</td>" : "") +
  (!isFuelContainerTypeHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.FuelContainerType + @"</td>" : "") +
 (!isFuelContainerCountHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.ItemsCount + @"</td>" : "") +
      (!isMilDepartmentHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.MilitaryDepartment + @"</td>" : "") +
       (!isMilRepStatusHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.MilitaryReportStatus + @"</td>" : "") +
          (!isOwnershipHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.Ownership + @"</td>" : "") +
          (!isAddressHidden ? "<td style='" + cellStyle + @"'>" + fuelContainerManageBlock.Address + @"</td>" : "") + @"
        ...
        private ReportFuelContainerManageFilter CollectFilterData()
        {
