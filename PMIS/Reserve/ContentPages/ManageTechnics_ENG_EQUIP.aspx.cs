...
{
    public partial class ManageTechnics_ENG_EQUIP : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            EngEquipManageFilter filter = CollectFilterData();

            int allRows = EngEquipUtil.GetAllEngEquipManageBlocksCount(filter, CurrentUser);
            ...
            if (!IsPostBack)
            {
                ...
                this.btnPrintAllEngEquips.Visible = false;
        ...
        private void RefreshDataGrid()
        {
            ...
            List<EngEquipManageBlock> engEquipManageBlocks = EngEquipUtil.GetAllEngEquipManageBlocks(filter, pageLength, CurrentUser);
            ...
            {
                this.btnPrintAllEngEquips.Visible = true;
                ...
                html = @"<table class='CommonHeaderTable' style='margin: 0 auto; text-align: left;'>
                         <thead>
                            <tr>
                               ...
                               <th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(5);'>Марка (базова машина)" + arrOrderCol[4] + @"</th>
                               <th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(6);'>Модел (базова машина)" + arrOrderCol[5] + @"</th>
                ...
                foreach (EngEquipManageBlock engEquipManageBlock in engEquipManageBlocks)
                {
                    ...
                    if (GetUIItemAccessLevel("RES_TECHNICS_ENG_EQUIP_EDIT") != UIAccessLevel.Hidden)
                        editHTML = "<img src='../Images/edit.png' alt='Редактиране' title='Редактиране' class='GridActionIcon' onclick='EditTechnics(" + engEquipManageBlock.TechnicsId.ToString() + ");' />";

                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"'>
                                 <td style='" + cellStyle + @"'>" + ((pageIdx - 1) * pageLength + counter).ToString() + @"</td>
                                 <td style='" + cellStyle + @"' title='" + engEquipManageBlock.NormativeTechnicsName + @"'>" + engEquipManageBlock.NormativeTechnicsCode + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.RegNumber + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.InventoryNumber + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.TechnicsCategory + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.EngEquipKind + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.EngEquipBaseMakeName + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.EngEquipBaseModelName + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.MilitaryDepartment + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.MilitaryReportStatus + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.Ownership + @"</td>
                                 <td style='" + cellStyle + @"'>" + engEquipManageBlock.Address + @"</td>
        ...
        {          
            if (GetUIItemAccessLevel("RES_TECHNICS") == UIAccessLevel.Enabled &&
                GetUIItemAccessLevel("RES_TECHNICS_ENG_EQUIP") == UIAccessLevel.Enabled &&
                GetUIItemAccessLevel("RES_TECHNICS_ENG_EQUIP_ADD") == UIAccessLevel.Enabled)
            {
                EnableButton(btnNew);
            }
            else
            {
                if (this.GetUIItemAccessLevel("RES_TECHNICS_ENG_EQUIP_ADD") == UIAccessLevel.Hidden)
                {
                    HideControl(btnNew);
                }
                else
        ...
        private EngEquipManageFilter CollectFilterData()
        {
