...
{
    public partial class ManageEquipmentTechnicsRequestsFulfilment : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL") == UIAccessLevel.Hidden)
            {
            ...
            HighlightMenuItems("Equipment", "ManageEquipmentTechnicsRequestsFulfilment");
            ...
            int allRows = EquipmentTechnicsRequestUtil.GetAllEquipmentTechnicsRequestForFulfilmentCount(filter, CurrentUser);
        ...
        private void RefreshEquipmentTechnicsRequests()
        {
            ...
            bool IsRequestNumberHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_REQUEST_REQUESTNUMBER") == UIAccessLevel.Hidden;
            bool IsRequestDateHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_REQUEST_REQUESTDATE") == UIAccessLevel.Hidden;
            bool IsMilitaryUnitHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_REQUEST_MILITARYUNIT") == UIAccessLevel.Hidden;
            bool IsAdministrationHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_REQUEST_ADMINISTRATION") == UIAccessLevel.Hidden;
            bool IsStatusHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_REQUEST_STATUS") == UIAccessLevel.Hidden;
            ...
            List<EquipmentTechnicsRequest> equipmentTechnicsRequests = EquipmentTechnicsRequestUtil.GetAllEquipmentTechnicsRequestForFulfilment(filter, pageLength, CurrentUser);
  ...
  (!IsAdministrationHidden ? @"<th style='width: 250px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(4);'>Министерство/Ведомство" + arrOrderCol[3] + @"</th>" : "") +
          (!IsStatusHidden ? @"<th style='width: 200px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(5);'>Статус на заявката" + arrOrderCol[4] + @"</th>" : "") +
                ...
                foreach (EquipmentTechnicsRequest equipmentTechnicsRequest in equipmentTechnicsRequests)
                {
                    ...
                    if (GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL") != UIAccessLevel.Hidden)
                        fulfilmentHTML = "<img src='../Images/list_edit.png' alt='Изпълнение на заявка' title='Изпълнение на заявка' class='GridActionIcon' onclick='FulfilEquipmentTechnics(" + equipmentTechnicsRequest.EquipmentTechnicsRequestId.ToString() + ", " + ddMilitaryDepartment.SelectedValue + ");' />";

                    string fulFil = "";
                    decimal fulFilPerc = 0;
                    decimal fulFilResPerc = 0;

                    if (equipmentTechnicsRequest.Count > 0)
                    {
                        fulFilPerc = (decimal)equipmentTechnicsRequest.FulfilCount / (decimal)equipmentTechnicsRequest.Count * (decimal)100;
                        fulFilResPerc = (decimal)equipmentTechnicsRequest.FulfilResCount / (decimal)equipmentTechnicsRequest.Count * (decimal)100;
                    }

                    fulFil = "<span title='" + ReadinessUtil.ReadinessName(1) + "' style='cursor: default;'>" + fulFilPerc.ToString("0.0") + "%</span> <span title='" + ReadinessUtil.ReadinessName(2) + "' style='cursor: default;'>(" + fulFilResPerc.ToString("0.0") + "%)</span>";

                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"'>
                                 <td style='" + cellStyle + @"'>" + ((pageIdx - 1) * pageLength + counter).ToString() + @"</td> " +
     (!IsRequestNumberHidden ? @"<td style='" + cellStyle + @"'>" + equipmentTechnicsRequest.RequestNumber + @"</td>" : "") +
       (!IsRequestDateHidden ? @"<td style='" + cellStyle + @"'>" + CommonFunctions.FormatDate(equipmentTechnicsRequest.RequestDate) + @"</td>" : "") +
      (!IsMilitaryUnitHidden ? @"<td style='" + cellStyle + @"'>" + (equipmentTechnicsRequest.MilitaryUnit == null ? "" : equipmentTechnicsRequest.MilitaryUnit.DisplayTextForSelection) + @"</td>" : "") +
    (!IsAdministrationHidden ? @"<td style='" + cellStyle + @"'>" + (equipmentTechnicsRequest.Administration == null ? "" : equipmentTechnicsRequest.Administration.AdministrationName) + @"</td>" : "") +
            (!IsStatusHidden ? @"<td style='" + cellStyle + @"'>" + (equipmentTechnicsRequest.EquipWithTechRequestsStatus == null ? "" : equipmentTechnicsRequest.EquipWithTechRequestsStatus.StatusName) + @"</td>" : "") +
