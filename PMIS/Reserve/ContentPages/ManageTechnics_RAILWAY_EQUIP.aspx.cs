...
{
    public partial class ManageTechnics_RAILWAY_EQUIP : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            RailwayEquipManageFilter filter = CollectFilterData();

            int allRows = RailwayEquipUtil.GetAllRailwayEquipManageBlocksCount(filter, CurrentUser);
        ...
        private void RefreshDataGrid()
        {
            ...
            List<RailwayEquipManageBlock> railwayEquipManageBlocks = RailwayEquipUtil.GetAllRailwayEquipManageBlocks(filter, pageLength, CurrentUser);

            //No data found
            if (railwayEquipManageBlocks.Count == 0)
            {
                html = "<span>Няма намерени резултати</span>";
                this.btnPrintAllRailwayEquips.Visible = false;
                this.btnExport.Visible = false;
            }
            //If there is data then generate dynamically the HTML for the data grid
            else 
            {
                this.btnPrintAllRailwayEquips.Visible = true;
                ...
                foreach (RailwayEquipManageBlock railwayEquipManageBlock in railwayEquipManageBlocks)
                {
                    ...
                    if (GetUIItemAccessLevel("RES_TECHNICS_RAILWAY_EQUIP_EDIT") != UIAccessLevel.Hidden)
                        editHTML = "<img src='../Images/edit.png' alt='Редактиране' title='Редактиране' class='GridActionIcon' onclick='EditTechnics(" + railwayEquipManageBlock.TechnicsId.ToString() + ");' />";

                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"'>
                                 <td style='" + cellStyle + @"'>" + ((pageIdx - 1) * pageLength + counter).ToString() + @"</td>
                                 <td style='" + cellStyle + @"' title='" + railwayEquipManageBlock.NormativeTechnicsName + @"'>" + railwayEquipManageBlock.NormativeTechnicsCode + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.InventoryNumber + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.TechnicsCategory + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.RailwayEquipKind + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.RailwayEquipType + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.ItemsCount + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.MilitaryDepartment + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.MilitaryReportStatus + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.Ownership + @"</td>
                                 <td style='" + cellStyle + @"'>" + railwayEquipManageBlock.Address + @"</td>
        ...
        {           
            if (GetUIItemAccessLevel("RES_TECHNICS") == UIAccessLevel.Enabled &&
                GetUIItemAccessLevel("RES_TECHNICS_RAILWAY_EQUIP") == UIAccessLevel.Enabled &&
                GetUIItemAccessLevel("RES_TECHNICS_RAILWAY_EQUIP_ADD") == UIAccessLevel.Enabled)
            {
                EnableButton(btnNew);
            }
            else
            {
                if (this.GetUIItemAccessLevel("RES_TECHNICS_RAILWAY_EQUIP_ADD") == UIAccessLevel.Hidden)
                {
                    HideControl(btnNew);
                }
                else
        ...
        private RailwayEquipManageFilter CollectFilterData()
        {
