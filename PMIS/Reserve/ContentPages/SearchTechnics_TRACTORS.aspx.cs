...
{
    public partial class SearchTechnics_TRACTORS : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.Params["AjaxMethod"] != null && Request.Params["AjaxMethod"] == "JSChooseTractor")
            {
                JSChooseTractor();
                return;
            }
            ...
            TractorSearchFilter filter = CollectFilterData();

            int allRows = TractorUtil.GetAllTractorSearchBlocksCount(filter, TechnicsRequestCommandPositionID, MilitaryDepartmentId, CurrentUser);
        ...
        private void RefreshDataGrid()
        {
            ...
            bool IsNormativeTechnicsHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_NORMATIVETECHNICS") == UIAccessLevel.Hidden;
            bool IsRegNumberHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_REGNUMBER") == UIAccessLevel.Hidden;
            bool IsInventoryNumberHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_INVENTNUMBER") == UIAccessLevel.Hidden;
            bool IsTechnicsCategoryHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_TECHNICSCATEGORY") == UIAccessLevel.Hidden;
            bool IsTractorKindHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_TRACTORKIND") == UIAccessLevel.Hidden;
            bool IsTractorMakeHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_TRACTORMAKE") == UIAccessLevel.Hidden;
            bool IsTractorModelHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_TRACTORMODEL") == UIAccessLevel.Hidden;
            bool IsOwnershipHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRACTOR_OWNERSHIP") == UIAccessLevel.Hidden;
            ...
            List<TractorSearchBlock> tractorSearchBlocks = TractorUtil.GetAllTractorSearchBlocks(filter, TechnicsRequestCommandPositionID, MilitaryDepartmentId, pageLength, CurrentUser);

            //No data found
            if (tractorSearchBlocks.Count == 0)
            {
 ...
 (!IsTechnicsCategoryHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(3);'>Категория" + arrOrderCol[2] + @"</th>" : "") +
     ...
     (!IsTractorModelHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(6);'>Модел" + arrOrderCol[5] + @"</th>" : "") +
        (!IsOwnershipHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(7);'>Собственик" + arrOrderCol[6] + @"</th>" : "") +
                ...
                foreach (TractorSearchBlock tractorSearchBlock in tractorSearchBlocks)
                {
                    ...
                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"' onmouseover=""this.className = 'SelectionItem';"" onmouseout=""this.className = '" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"';"">
                                 ...
                                 <td style='" + cellStyle + @"'><img src='../Images/pawn_view.png' alt='Преглед' title='Преглед' class='GridActionIcon' onclick='PreviewTechnics(" + tractorSearchBlock.TechnicsId.ToString() + @");' /></td>" +
 (!IsNormativeTechnicsHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='" + tractorSearchBlock.NormativeTechnicsName + @"'>" + tractorSearchBlock.NormativeTechnicsCode + @"</td>" : "") +
         (!IsRegNumberHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='Избери'>" + tractorSearchBlock.RegNumber + @"</td>" : "") +
   (!IsInventoryNumberHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='Избери'>" + tractorSearchBlock.InventoryNumber + @"</td>" : "") +
  (!IsTechnicsCategoryHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='Избери'>" + tractorSearchBlock.TechnicsCategory + @"</td>" : "") +
       (!IsTractorKindHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='Избери'>" + tractorSearchBlock.TractorKind + @"</td>" : "") +
       (!IsTractorMakeHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='Избери'>" + tractorSearchBlock.TractorMake + @"</td>" : "") +
      (!IsTractorModelHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='Избери'>" + tractorSearchBlock.TractorModel + @"</td>" : "") +
         (!IsOwnershipHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTractor(" + tractorSearchBlock.TechnicsId + @", " + tractorSearchBlock.TractorId + @");"" title='Избери'>" + tractorSearchBlock.Ownership + @"</td>" : "") +
                              @"</tr>";
        ...
        private TractorSearchFilter CollectFilterData()
        {
            TractorSearchFilter filter = new TractorSearchFilter();
        ...
        private void JSChooseTractor()
        {
            string stat = "";
            string response = "";

            int technicsID = int.Parse(Request.Form["TechnicsID"]);
            int tractorID = int.Parse(Request.Form["TractorID"]);
            ...
            try
            {
                ...
                if (existingFulfillments.Count == 0)
                {
                    ...
                    Tractor tractor = TractorUtil.GetTractor(tractorID, CurrentUser);
                    ...
                    changeEvent = new ChangeEvent("RES_EquipTechRequests_AddTractor", logDescription, position.TechnicsRequestsCommand.EquipmentTechnicsRequest.MilitaryUnit, null, CurrentUser);
                    ...
                    //Add a new TechnicsAppointment for the chosen tractor
