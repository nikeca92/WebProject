var techFulfilmentRemoval = (function() {
    ...
    var getData = function(widgetId, callbackFunction, requestCommantID, militaryDepartmentID) {
        ...
        var url = "TechFulfilmentRemovalHandlers.aspx?AjaxMethod=JSGetData";
    ...
    var extractData = function(xml) {
        ...
        data.technicsRequestCommandPositions = [];
        var technicsRequestCommandPositions = xml.getElementsByTagName("TechnicsRequestCommandPosition");

        if (technicsRequestCommandPositions.length > 0) {
            for (var i = 0; i < technicsRequestCommandPositions.length; i++) {

                var technicsRequestCommandPositionID = xmlValue(technicsRequestCommandPositions[i], "TechnicsRequestCommandPositionID");
                var technicsType = xmlValue(technicsRequestCommandPositions[i], "TechnicsType");
                var normativeTechnics = xmlValue(technicsRequestCommandPositions[i], "NormativeTechnics");
                var technicsComment = xmlValue(technicsRequestCommandPositions[i], "TechnicsComment");
                var technicsCount = xmlValue(technicsRequestCommandPositions[i], "TechnicsCount");
                var fulfiled = xmlValue(technicsRequestCommandPositions[i], "Fulfiled");

                data.technicsRequestCommandPositions.push({ technicsRrequestCommandPositionID: technicsRequestCommandPositionID,
                    technicsType: technicsType,
                    normativeTechnics: normativeTechnics,
                    technicsComment: technicsComment,
                    technicsCount: technicsCount,
                    fulfiled: fulfiled
                });
            }
        }
    ...
    var setupLightboxPnlHTML = function(lightBox, data) {
        var html = "<div id='" + lightBox.id + "_pnl' style='min-height:510px;'> " +
                     ...
                     "                  Прекратяване на МН на техника от команда: <span style=\"color:#000000;\">" + data.requestCommand.requestCommandName + "</span>," +
                     ...
                     "                  <thead>" +
                     "                  <tr>" +
                     "                      <th><input type=\"checkbox\" title=\"Избери всички\" id=\"" + lightBox.id + "_chkAll\" onclick=\"techFulfilmentRemoval.chkAll_Click('" + lightBox.id + "')\"/></th>" +
                     "                      <th>Вид техника</th>" +
                     "                      <th>Нормативна категория</th>" +
                     //"                      <th>Коментар</th>" +
                     "                      <th>Заявени</th>" +
                     "                      <th>Изпълнени</th>" +
                     "                  </tr>" +
        ...
        for (var i = 0; i < data.technicsRequestCommandPositions.length; i++) {

            html += "                   <tr>" +
                    "                       <td id='" + lightBox.id + "_tr_" + (i + 1) + "_td_1' style=\"text-align:center; vertical-align: text-top;\">" +
                    "                           <input id=\"" + lightBox.id + "_chkTechnicsRequestCommandPosition_" + (i + 1) + "\" type=\"checkbox\" data-tr=\"" + (i + 1) + "\" onclick=\"techFulfilmentRemoval.chk_Click(this, '" + lightBox.id + "' )\"/>" +
                    "                           <input type='hidden' id='" + lightBox.id + "_technicsRequestCommandPositionID_" + (i + 1) + "' value='" + data.technicsRequestCommandPositions[i].technicsRrequestCommandPositionID + "' />" +
                    "                       </td>" +
                    "                       <td id='" + lightBox.id + "_tr_" + (i + 1) + "_td_2' style='text-align:left; vertical-align: text-top;word-break:break-all;'>" + data.technicsRequestCommandPositions[i].technicsType + "</td>" +
                    "                       <td id='" + lightBox.id + "_tr_" + (i + 1) + "_td_3' style='text-align:left;word-break:break-all;'>" + data.technicsRequestCommandPositions[i].normativeTechnics + "</td>" +
                    //"                       <td id='" + lightBox.id + "_tr_" + (i + 1) + "_td_4' > " + data.technicsRequestCommandPositions[i].technicsComment + "</td>" +
                    "                       <td id='" + lightBox.id + "_tr_" + (i + 1) + "_td_4' >" + data.technicsRequestCommandPositions[i].technicsCount + "</td>" +
                    "                       <td id='" + lightBox.id + "_tr_" + (i + 1) + "_td_5' >" + data.technicsRequestCommandPositions[i].fulfiled + "</td>" +
                    "                   </tr>";
        }
        ...
        html += "                    </table>" +
               ...
               "                                   <div id=\"" + lightBox.id + "_btnRemove\" style=\"display: inline;\" onclick='techFulfilmentRemoval.btnRemove_Click(\"" + lightBox.id + "\");' class=\"Button\">" +
               "                                       <i></i>" +
               "                                       <div id=\"" + lightBox.id + "_btnRemoveText\" style=\"width: 170px;\">Прекратяване на МН</div>" +
               "                                       <b></b>" +
               "                                   </div>" +
               "                                   &nbsp;" +
               "                                   <div id=\"" + lightBox.id + "_btnBack\" style=\"display: inline;\" onclick=\"techFulfilmentRemoval.hideDialog('" + lightBox.id + "');\" class=\"Button\">" +
               "                                       <i></i>" +
               ...
               "            </tr>" +
               "        </table>" +
               "        <input type='hidden' id='" + lightBox.id + "_technicsRequestCommandID' value='" + data.requestCommand.requestCommandID + "' />" +
               "        <input type='hidden' id='" + lightBox.id + "_technicsRequestCommandPositionsCnt' value='" + data.technicsRequestCommandPositions.length + "' />" +
               "    </div>";
    ...
    function toggleAll(widgetId) {
        ...
        var requestCommandPositionsCnt = parseInt(document.getElementById(widgetId + "_technicsRequestCommandPositionsCnt").value);
        ...
        for (var i = 0; i < requestCommandPositionsCnt; i++) {
            ...
            document.getElementById(widgetId + "_chkTechnicsRequestCommandPosition_" + (i + 1)).checked = chk.checked;
        }
    }

    //remove data
    function removeData(widgetId) {
        var url = "TechFulfilmentRemovalHandlers.aspx?AjaxMethod=JSRemoveTechnics";
        var params = "";

        var technicsRequestCommandPositionsCnt = parseInt(document.getElementById(widgetId + "_technicsRequestCommandPositionsCnt").value);

        var IDs = "";
        for (var i = 0; i < technicsRequestCommandPositionsCnt; i++) {
            var chk = document.getElementById(widgetId + "_chkTechnicsRequestCommandPosition_" + (i + 1));

            if (chk.checked) {
                IDs += (IDs == "" ? "" : ",") + document.getElementById(widgetId + "_technicsRequestCommandPositionID_" + (i + 1)).value;                                
            }
        }
        
        params = "TechnicsRequestCommandPositionIDs=" + IDs;
