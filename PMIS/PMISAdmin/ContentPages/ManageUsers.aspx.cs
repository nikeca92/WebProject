...
{
    public partial class ManageUsers : AdmPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.Params["AjaxMethod"] != null && Request.Params["AjaxMethod"] == "JSDeleteUser")
            {
                JSDeleteUser();
                return;
            }
            ...
            HighlightMenuItems("Users", "Users_ManageUsers");
            ...
            int allRows = UserUtil.GetUsersCnt(CurrentUser, txtFullName.Text, txtUsername.Text, int.Parse(ddShow.SelectedValue));
            ...
            if (!IsPostBack)
            {
                ...
                //The default order is by first name
        ...
        {
            lblFullName.Style.Add("vertical-align", "top");
            txtFullName.Style.Add("vertical-align", "top");
            lblUsername.Style.Add("vertical-align", "top");
            txtUsername.Style.Add("vertical-align", "top");
            lblShow.Style.Add("vertical-align", "top");
            ddShow.Style.Add("vertical-align", "top");
        }
        ...
        private void RefreshUsers()
        {
            ...
            List<User> users = UserUtil.GetUsers(CurrentUser, txtFullName.Text, txtUsername.Text, int.Parse(ddShow.SelectedValue),
                                                 orderBy, pageIdx, pageLength);
            ...
            else
            {
                ...
                html = @"<table class='CommonHeaderTable' style='margin: 0 auto; text-align: left;'>
                         <thead>
                            <tr>
                               ...
                               <th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(1);'>Име" + arrOrderCol[0] + @"</th>
                               <th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(2);'>Презиме" + arrOrderCol[1] + @"</th>
                               <th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(3);'>Фамилия" + arrOrderCol[2] + @"</th>
                               <th style='width: 160px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(4);'>Потребителско име" + arrOrderCol[3] + @"</th>
                ...
                foreach (User user in users)
                {
                    ...
                    if (!user.IsActive)
                        cellStyle += "color: #999999; font-style: italic;";

                    string deleteHTML = "";
                    string editHTML = "";
                    string milStructureAccessHTML = "";

                    if (user.CanDelete && CurrentUser.UserId != user.UserId)
                    {
                        if (GetUIItemAccessLevel("ADM_SECURITY") == UIAccessLevel.Enabled &&
                            GetUIItemAccessLevel("ADM_SECURITY_USERS") == UIAccessLevel.Enabled &&
                            GetUIItemAccessLevel("ADM_SECURITY_USERS_DELETEUSER") == UIAccessLevel.Enabled
                            )
                            deleteHTML = "<img src='../Images/delete.png' alt='Изтриване' title='Изтриване на този потребител' class='GridActionIcon' onclick='DeleteUser(" + user.UserId.ToString() + ");' />";
                    }

                    if (GetUIItemAccessLevel("ADM_SECURITY_USERS_EDITUSER") != UIAccessLevel.Hidden)
                        editHTML = "<img src='../Images/edit.png' alt='Редактиране' title='Редактиране' class='GridActionIcon' onclick='EditUser(" + user.UserId.ToString() + ");' />";

                    if (GetUIItemAccessLevel("ADM_SECURITY_USERS_MILSTRUCTUREACCESS") != UIAccessLevel.Hidden)
                        milStructureAccessHTML = "<img src='../Images/key.png' alt='Дефиниране права на достъп до военни структури' title='Дефиниране права на достъп до военни структури' class='GridActionIcon' onclick='MilitaryStructureAccessPerUser(" + user.UserId.ToString() + ");' />";

                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"'>
                                 <td style='" + cellStyle + @"'>" + ((pageIdx - 1) * pageLength + counter).ToString() + @"</td>
                                 <td style='" + cellStyle + @"'>" + user.FirstName + @"</td>
                                 <td style='" + cellStyle + @"'>" + user.MiddleName + @"</td>
                                 <td style='" + cellStyle + @"'>" + user.LastName + @"</td>
                                 <td style='" + cellStyle + @"'>" + user.Username + @"</td>
                                 <td style='" + cellStyle + @"'>" + editHTML + milStructureAccessHTML + deleteHTML + @"</td>
                              </tr>";
            ...
            if (hdnRefreshReason.Value != "")
            {
                ...
                {
                    lblGridMessage.Text = "Потребителят беше изтрит успешно";
        ...
        {
            Response.Redirect("~/ContentPages/AddEditUser.aspx");
        }

        //Delete user (ajax call)
        private void JSDeleteUser()
        {
            if (GetUIItemAccessLevel("ADM_SECURITY") != UIAccessLevel.Enabled ||
                GetUIItemAccessLevel("ADM_SECURITY_USERS") != UIAccessLevel.Enabled ||
                GetUIItemAccessLevel("ADM_SECURITY_USERS_DELETEUSER") != UIAccessLevel.Enabled)
            ...
            try
            {
                ...
                UserUtil.DeleteUser(CurrentUser, userId, change);
