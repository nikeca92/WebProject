...
{
    public partial class ReportListReservistsWithAppointments : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ReportListReservistsWithAppointmentsFilter filter = CollectFilterData();

            int allRows = ReportListReservistsWithAppointmentsBlockUtil.GetReportListReservistsWithAppointmentsBlockCount(filter, CurrentUser);
            ...
            if (!IsPostBack)
            {
                ...
                this.btnPrintReportListReservistsWithAppointments.Visible = false;
        ...
        private void RefreshReportItems()
        {
            ...
            List<ReportListReservistsWithAppointmentsBlock> reportBlocks = ReportListReservistsWithAppointmentsBlockUtil.GetReportListReservistsWithAppointmentsBlockList(filter, pageLength, CurrentUser);
            ...
            {
                this.btnPrintReportListReservistsWithAppointments.Visible = true;
  ...
  (!isAdministrationHidden ? @"<th style='width: 250px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(4);'>Министерство/Ведомство" + arrOrderCol[3] + @"</th>" : "") +
       (!isReqStatusHidden ? @"<th style='width: 200px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(5);'>Статус на заявката" + arrOrderCol[4] + @"</th>" : "") + @"
                ...
                foreach (ReportListReservistsWithAppointmentsBlock reportBlock in reportBlocks)
                {
                    string cellStyle = "vertical-align: top;";

                    string detailsHTML = "";

                    detailsHTML = "<img src='../Images/note_view.png' alt='Детайлна справка' title='Детайлна справка' class='GridActionIcon' style='width: 20px; height: 20px;' onclick='ShowDetails(" + reportBlock.EquipmentReservistsRequestId.ToString() + ");' />";
    ...
    (!isAdministrationHidden ? @"<td style='" + cellStyle + @"'>" + reportBlock.Administration + @"</td>" : "") +
        ...
        private ReportListReservistsWithAppointmentsFilter CollectFilterData()
        {
            ...
            if (ddEquipWithResRequestsStatus.SelectedValue != ListItems.GetOptionAll().Value)
            {
                ...
                this.hdnEquipWithResRequestsStatusId.Value = ddEquipWithResRequestsStatus.SelectedValue;
            }
