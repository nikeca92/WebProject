...
{
    public partial class SearchTechnics_FUEL_CONTAINERS : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL") != UIAccessLevel.Enabled) ||
                (GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHFUELCONTAINER") != UIAccessLevel.Enabled))
            {
                RedirectAccessDenied();
            }

            //Check if this is an ajax call and perform the specific method
            if (Request.Params["AjaxMethod"] != null && Request.Params["AjaxMethod"] == "JSChooseFuelContainer")
            {
                JSChooseFuelContainer();
                return;
            }
            ...
            lblHeaderTitle.InnerHtml = "Търсене на " + TechnicsTypeName + " за попълване на заявка №" + request.RequestNumber + " / " + CommonFunctions.FormatDate(request.RequestDate);
            ...
            FuelContainerSearchFilter filter = CollectFilterData();

            int allRows = FuelContainerUtil.GetAllFuelContainerSearchBlocksCount(filter, TechnicsRequestCommandPositionID, MilitaryDepartmentId, CurrentUser);
        ...
        private void RefreshDataGrid()
        {
            ...
            bool IsNormativeTechnicsHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHFUELCONTAINER_NORMATIVETECHNICS") == UIAccessLevel.Hidden;
            bool IsInventoryNumberHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHFUELCONTAINER_INVENTNUMBER") == UIAccessLevel.Hidden;
            bool IsTechnicsCategoryHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHFUELCONTAINER_TECHNICSCATEGORY") == UIAccessLevel.Hidden;
            bool IsFuelContainerKindHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHFUELCONTAINER_FUELCONTAINERKIND") == UIAccessLevel.Hidden;
            bool IsFuelContainerTypeHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHFUELCONTAINER_FUELCONTAINERTYPE") == UIAccessLevel.Hidden;
            bool IsOwnershipHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHFUELCONTAINER_OWNERSHIP") == UIAccessLevel.Hidden;
            ...
            List<FuelContainerSearchBlock> fuelContainerSearchBlocks = FuelContainerUtil.GetAllFuelContainerSearchBlocks(filter, TechnicsRequestCommandPositionID, MilitaryDepartmentId, pageLength, CurrentUser);

            //No data found
            if (fuelContainerSearchBlocks.Count == 0)
            {
...
(!IsFuelContainerKindHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(3);'>Вид" + arrOrderCol[2] + @"</th>" : "") +
(!IsFuelContainerTypeHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(4);'>Тип" + arrOrderCol[3] + @"</th>" : "") +
        ...
        (!IsOwnershipHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(6);'>Собственик" + arrOrderCol[5] + @"</th>" : "") + @"
                ...
                foreach (FuelContainerSearchBlock fuelContainerSearchBlock in fuelContainerSearchBlocks)
                {
                    ...
                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"' onmouseover=""this.className = 'SelectionItem';"" onmouseout=""this.className = '" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"';"">
                                 ...
                                 <td style='" + cellStyle + @"'><img src='../Images/pawn_view.png' alt='Преглед' title='Преглед' class='GridActionIcon' onclick='PreviewTechnics(" + fuelContainerSearchBlock.TechnicsId.ToString() + @");' /></td>" +
 (!IsNormativeTechnicsHidden ? @"<td style='" + cellStyle + @"' onclick=""ShowChooseItemsCountLightBox(" + fuelContainerSearchBlock.TechnicsId + @", " + fuelContainerSearchBlock.FuelContainerId + @", " + fuelContainerSearchBlock.ItemsCount + @");"" title='" + fuelContainerSearchBlock.NormativeTechnicsName + @"'>" + fuelContainerSearchBlock.NormativeTechnicsCode + @"</td>" : "") +
   (!IsInventoryNumberHidden ? @"<td style='" + cellStyle + @"' onclick=""ShowChooseItemsCountLightBox(" + fuelContainerSearchBlock.TechnicsId + @", " + fuelContainerSearchBlock.FuelContainerId + @", " + fuelContainerSearchBlock.ItemsCount + @");"" title='Избери'>" + fuelContainerSearchBlock.InventoryNumber + @"</td>" : "") +
  (!IsTechnicsCategoryHidden ? @"<td style='" + cellStyle + @"' onclick=""ShowChooseItemsCountLightBox(" + fuelContainerSearchBlock.TechnicsId + @", " + fuelContainerSearchBlock.FuelContainerId + @", " + fuelContainerSearchBlock.ItemsCount + @");"" title='Избери'>" + fuelContainerSearchBlock.TechnicsCategory + @"</td>" : "") +
 (!IsFuelContainerKindHidden ? @"<td style='" + cellStyle + @"' onclick=""ShowChooseItemsCountLightBox(" + fuelContainerSearchBlock.TechnicsId + @", " + fuelContainerSearchBlock.FuelContainerId + @", " + fuelContainerSearchBlock.ItemsCount + @");"" title='Избери'>" + fuelContainerSearchBlock.FuelContainerKind + @"</td>" : "") +
 (!IsFuelContainerTypeHidden ? @"<td style='" + cellStyle + @"' onclick=""ShowChooseItemsCountLightBox(" + fuelContainerSearchBlock.TechnicsId + @", " + fuelContainerSearchBlock.FuelContainerId + @", " + fuelContainerSearchBlock.ItemsCount + @");"" title='Избери'>" + fuelContainerSearchBlock.FuelContainerType + @"</td>" : "") +
                               @"<td style='" + cellStyle + @"' onclick=""ShowChooseItemsCountLightBox(" + fuelContainerSearchBlock.TechnicsId + @", " + fuelContainerSearchBlock.FuelContainerId + @", " + fuelContainerSearchBlock.ItemsCount + @");"" title='Избери'>" + fuelContainerSearchBlock.ItemsCount + @"</td>" +
         (!IsOwnershipHidden ? @"<td style='" + cellStyle + @"' onclick=""ShowChooseItemsCountLightBox(" + fuelContainerSearchBlock.TechnicsId + @", " + fuelContainerSearchBlock.FuelContainerId + @", " + fuelContainerSearchBlock.ItemsCount + @");"" title='Избери'>" + fuelContainerSearchBlock.Ownership + @"</td>" : "") + @"
                              </tr>";
        ...
        private FuelContainerSearchFilter CollectFilterData()
        {
            FuelContainerSearchFilter filter = new FuelContainerSearchFilter();
        ...
        private void JSChooseFuelContainer()
        {
            string stat = "";
            string response = "";

            int technicsID = int.Parse(Request.Form["TechnicsID"]);
            int fuelContainerID = int.Parse(Request.Form["FuelContainerID"]);
            ...
            try
            {
                ...
                if (existingFulfillments.Count == 0)
                {
                    ...
                    {
                        FuelContainerUtil.SplitFuelContainer(technicsID, itemsCount, CurrentUser, change);
                    }
                    ...
                    FuelContainer fuelContainer = FuelContainerUtil.GetFuelContainer(fuelContainerID, CurrentUser);
                    ...
                    changeEvent = new ChangeEvent("RES_EquipTechRequests_AddFuelContainer", logDescription, position.TechnicsRequestsCommand.EquipmentTechnicsRequest.MilitaryUnit, null, CurrentUser);
