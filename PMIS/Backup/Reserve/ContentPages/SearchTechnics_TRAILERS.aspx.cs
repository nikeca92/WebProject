...
{
    public partial class SearchTechnics_TRAILERS : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL") != UIAccessLevel.Enabled) ||
                (GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER") != UIAccessLevel.Enabled))
            {
                RedirectAccessDenied();
            }

            //Check if this is an ajax call and perform the specific method
            if (Request.Params["AjaxMethod"] != null && Request.Params["AjaxMethod"] == "JSChooseTrailer")
            {
                JSChooseTrailer();
                return;
            }
            ...
            TrailerSearchFilter filter = CollectFilterData();

            int allRows = TrailerUtil.GetAllTrailerSearchBlocksCount(filter, TechnicsRequestCommandPositionID, MilitaryDepartmentId, CurrentUser);
        ...
        private void RefreshDataGrid()
        {
            ...
            bool IsNormativeTechnicsHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER_NORMATIVETECHNICS") == UIAccessLevel.Hidden;
            bool IsRegNumberHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER_REGNUMBER") == UIAccessLevel.Hidden;
            bool IsInventoryNumberHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER_INVENTNUMBER") == UIAccessLevel.Hidden;
            bool IsTechnicsCategoryHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER_TECHNICSCATEGORY") == UIAccessLevel.Hidden;
            bool IsTrailerKindHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER_TRAILERKIND") == UIAccessLevel.Hidden;
            bool IsTrailerTypeHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER_TRAILERTYPE") == UIAccessLevel.Hidden;
            bool IsOwnershipHidden = this.GetUIItemAccessLevel("RES_EQUIPTECHREQUESTS_FULFIL_MILCOMMAND_POSITIONS_FULFIL_SEARCHTRAILER_OWNERSHIP") == UIAccessLevel.Hidden;
            ...
            List<TrailerSearchBlock> trailerSearchBlocks = TrailerUtil.GetAllTrailerSearchBlocks(filter, TechnicsRequestCommandPositionID, MilitaryDepartmentId, pageLength, CurrentUser);

            //No data found
            if (trailerSearchBlocks.Count == 0)
            {
...
(!IsNormativeTechnicsHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(7);'>Нормативна категория" + arrOrderCol[6] + @"</th>" : "") +
 ...
 (!IsTechnicsCategoryHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(3);'>Категория" + arrOrderCol[2] + @"</th>" : "") +
      (!IsTrailerKindHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(4);'>Вид" + arrOrderCol[3] + @"</th>" : "") +
      (!IsTrailerTypeHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(5);'>Тип" + arrOrderCol[4] + @"</th>" : "") +
        (!IsOwnershipHidden ? @"<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(6);'>Собственик" + arrOrderCol[5] + @"</th>" : "") +
                ...
                foreach (TrailerSearchBlock trailerSearchBlock in trailerSearchBlocks)
                {
                    ...
                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"' onmouseover=""this.className = 'SelectionItem';"" onmouseout=""this.className = '" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"';"">
                                 ...
                                 <td style='" + cellStyle + @"'><img src='../Images/pawn_view.png' alt='Преглед' title='Преглед' class='GridActionIcon' onclick='PreviewTechnics(" + trailerSearchBlock.TechnicsId.ToString() + @");' /></td>" +
 (!IsNormativeTechnicsHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTrailer(" + trailerSearchBlock.TechnicsId + @", " + trailerSearchBlock.TrailerId + @");"" title='" + trailerSearchBlock.NormativeTechnicsName + @"'>" + trailerSearchBlock.NormativeTechnicsCode + @"</td>" : "") +
         (!IsRegNumberHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTrailer(" + trailerSearchBlock.TechnicsId + @", " + trailerSearchBlock.TrailerId + @");"" title='Избери'>" + trailerSearchBlock.RegNumber + @"</td>" : "") +
   (!IsInventoryNumberHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTrailer(" + trailerSearchBlock.TechnicsId + @", " + trailerSearchBlock.TrailerId + @");"" title='Избери'>" + trailerSearchBlock.InventoryNumber + @"</td>" : "") +
  (!IsTechnicsCategoryHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTrailer(" + trailerSearchBlock.TechnicsId + @", " + trailerSearchBlock.TrailerId + @");"" title='Избери'>" + trailerSearchBlock.TechnicsCategory + @"</td>" : "") +
       (!IsTrailerKindHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTrailer(" + trailerSearchBlock.TechnicsId + @", " + trailerSearchBlock.TrailerId + @");"" title='Избери'>" + trailerSearchBlock.TrailerKind + @"</td>" : "") +
       (!IsTrailerTypeHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTrailer(" + trailerSearchBlock.TechnicsId + @", " + trailerSearchBlock.TrailerId + @");"" title='Избери'>" + trailerSearchBlock.TrailerType + @"</td>" : "") +
         (!IsOwnershipHidden ? @"<td style='" + cellStyle + @"' onclick=""ChooseTrailer(" + trailerSearchBlock.TechnicsId + @", " + trailerSearchBlock.TrailerId + @");"" title='Избери'>" + trailerSearchBlock.Ownership + @"</td>" : "") +
                              @"</tr>";
        ...
        private TrailerSearchFilter CollectFilterData()
        {
            TrailerSearchFilter filter = new TrailerSearchFilter();
        ...
        private void JSChooseTrailer()
        {
            string stat = "";
            string response = "";

            int technicsID = int.Parse(Request.Form["TechnicsID"]);
            int trailerID = int.Parse(Request.Form["TrailerID"]);
            ...
            try
            {
                ...
                if (existingFulfillments.Count == 0)
                {
                    ...
                    Trailer trailer = TrailerUtil.GetTrailer(trailerID, CurrentUser);
                    ...
                    changeEvent = new ChangeEvent("RES_EquipTechRequests_AddTrailer", logDescription, position.TechnicsRequestsCommand.EquipmentTechnicsRequest.MilitaryUnit, null, CurrentUser);
                    ...
                    //Add a new TechnicsAppointment for the chosen trailer
