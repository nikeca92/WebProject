...
{
    public partial class ManageTechnics_VESSELS : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            VesselManageFilter filter = CollectFilterData();

            int allRows = VesselUtil.GetAllVesselManageBlocksCount(filter, CurrentUser);
        ...
        private void RefreshDataGrid()
        {
            ...
            List<VesselManageBlock> vesselManageBlocks = VesselUtil.GetAllVesselManageBlocks(filter, pageLength, CurrentUser);

            //No data found
            if (vesselManageBlocks.Count == 0)
            {
                html = "<span>Няма намерени резултати</span>";
                this.btnPrintAllVessels.Visible = false;
                this.btnExport.Visible = false;
            }
            //If there is data then generate dynamically the HTML for the data grid
            else 
            {
                this.btnPrintAllVessels.Visible = true;
                ...
                foreach (VesselManageBlock vesselManageBlock in vesselManageBlocks)
                {
                    ...
                    if (GetUIItemAccessLevel("RES_TECHNICS_VESSELS_EDIT") != UIAccessLevel.Hidden)
                        editHTML = "<img src='../Images/edit.png' alt='Редактиране' title='Редактиране' class='GridActionIcon' onclick='EditTechnics(" + vesselManageBlock.TechnicsId.ToString() + ");' />";

                    html += @"<tr class='" + (counter % 2 == 0 ? "DataTableEvenRow" : "DataTableOddRow") + @"'>
                                 <td style='" + cellStyle + @"'>" + ((pageIdx - 1) * pageLength + counter).ToString() + @"</td>
                                 <td style='" + cellStyle + @"' title='" + vesselManageBlock.NormativeTechnicsName + @"'>" + vesselManageBlock.NormativeTechnicsCode + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.VesselName + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.InventoryNumber + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.TechnicsCategory + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.VesselKind + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.VesselType + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.MilitaryDepartment + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.MilitaryReportStatus + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.Ownership + @"</td>
                                 <td style='" + cellStyle + @"'>" + vesselManageBlock.Address + @"</td>
        ...
        {            
            if (GetUIItemAccessLevel("RES_TECHNICS") == UIAccessLevel.Enabled &&
                GetUIItemAccessLevel("RES_TECHNICS_VESSELS") == UIAccessLevel.Enabled &&
                GetUIItemAccessLevel("RES_TECHNICS_VESSELS_ADD") == UIAccessLevel.Enabled)
            {
                EnableButton(btnNew);
            }
            else
            {
                if (this.GetUIItemAccessLevel("RES_TECHNICS_VESSELS_ADD") == UIAccessLevel.Hidden)
                {
                    HideControl(btnNew);
                }
                else
        ...
        private VesselManageFilter CollectFilterData()
        {
