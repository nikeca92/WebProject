...
{
    public partial class ReportTechnics_TRAILERS : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (GetUIItemAccessLevel("RES_REPORTS") == UIAccessLevel.Hidden ||
                GetUIItemAccessLevel("RES_REPORTS_TRAILERS") == UIAccessLevel.Hidden)
            {
            ...
            ReportTrailerManageFilter filter = CollectFilterData();

            int allRows = ReportTrailerUtil.GetAllReportTrailerManageBlocksCount(filter, CurrentUser);
            ...
            if (!IsPostBack)
            {
                ...
                this.btnPrintReportAllTrailers.Visible = false;
        ...
        private void RefreshDataGrid()
        {
            ...
            List<ReportTrailerManageBlock> reportTrailerManageBlocks = ReportTrailerUtil.GetAllReportTrailerManageBlocks(filter, pageLength, CurrentUser);
            ...
            {
                this.btnPrintReportAllTrailers.Visible = true;
...
(!isNormativeTechnicsHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(11);'>Нормативна категория" + arrOrderCol[10] + @"</th>" : "") +
 ...
 (!isTechnicsCategoryHidden ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(3);'>Категория" + arrOrderCol[2] + @"</th>" : "") +
      ...
      (!isBodyTypeHidden    ? "<th style='width: 150px; cursor: pointer;" + headerStyle + @"' onclick='SortTableBy(10);'>Каросерия" + arrOrderCol[9] + @"</th>" : "") +
  ...
  (!isNormativeTechnicsHidden ? "<td style='" + cellStyle + @"' title='" + reportTrailerManageBlock.NormativeTechnicsName + @"'>" + reportTrailerManageBlock.NormativeTechnicsCode + @"</td>" : "") +
          (!isRegNumberHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.RegNumber + @"</td>" : "") +
    (!isInventoryNumberHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.InventoryNumber + @"</td>" : "") +
   (!isTechnicsCategoryHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.TechnicsCategory + @"</td>" : "") +
        (!isTrailerKindHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.TrailerKind + @"</td>" : "") +
        (!isBodyTypeHidden    ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.BodyType + @"</td>" : "") +
        (!isTrailerTypeHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.TrailerType + @"</td>" : "") +
      (!isMilDepartmentHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.MilitaryDepartment + @"</td>" : "") +
       (!isMilRepStatusHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.MilitaryReportStatus + @"</td>" : "") +
          (!isOwnershipHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.Ownership + @"</td>" : "") +
          (!isAddressHidden ? "<td style='" + cellStyle + @"'>" + reportTrailerManageBlock.Address + @"</td>" : "") + @"
        ...
        private ReportTrailerManageFilter CollectFilterData()
        {
