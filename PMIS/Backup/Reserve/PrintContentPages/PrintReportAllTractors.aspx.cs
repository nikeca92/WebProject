...
{
    public partial class PrintReportAllTractors : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (GetUIItemAccessLevel("RES_TECHNICS") == UIAccessLevel.Hidden ||
                this.GetUIItemAccessLevel("RES_TECHNICS_TRACTORS") != UIAccessLevel.Hidden)
            {
                ...
                {
                    tractorKindId = Request.Params["TractorKindId"];
                }

                //if (!String.IsNullOrEmpty(Request.Params["TractorMakeId"]))
                //{
                //    tractorMakeId = Request.Params["TractorMakeId"];
                //}

                //if (!String.IsNullOrEmpty(Request.Params["TractorModelId"]))
                //{
                //    tractorModelId = Request.Params["TractorModelId"];
                //}

                if (!String.IsNullOrEmpty(Request.Params["TractorMakeName"]))
                {
                    tractorMakeName = Request.Params["TractorMakeName"];
                }

                if (!String.IsNullOrEmpty(Request.Params["TractorModelName"]))
                {
                    tractorModelName = Request.Params["TractorModelName"];
                }
        ...
        private string GenerateAllRecordsHtml()
        {
            ...
            List<ReportTractorManageBlock> reportTractorManageBlocks = ReportTractorUtil.GetAllReportTractorManageBlocks(filter, 0, CurrentUser);
            ...
            GTableItem tractorKind = null;
            int tracKindId = 0;
            if (int.TryParse(tractorKindId, out tracKindId))
            {
                tractorKind = GTableItemUtil.GetTableItem("TractorKind", tracKindId, ModuleUtil.RES(), CurrentUser);
            }

            //TractorMake tractorMake = null;
            //int tracMakeId = 0;
            //if (int.TryParse(tractorMakeId, out tracMakeId))
            //{
            //    tractorMake = TractorMakeUtil.GetTractorMake(tracMakeId, CurrentUser);
            //}

            //TractorModel tractorModel = null;
            //int tracModelId = 0;
            //if (int.TryParse(tractorModelId, out tracModelId))
            //{
            //    tractorModel = TractorModelUtil.GetTractorModel(tracModelId, CurrentUser);
            //}
            ...
            html.Append(@"<table style='padding: 5px; width: 1024px;'>
                             ...
                             <tr>
                                ...
                                <td align='left' style='width: 185px;'>
                                    <span class='ValueLabel'>" + (tractorKind != null ? tractorKind.TableValue : All) + @"</span>
                                </td>
                             </tr>
                             <tr>
                                <td align='right' style='width: 165px;'>
                                ...
                                <td align='left' style='width: 145px;'>
                                    <span class='ValueLabel'>" + tractorMakeName + @"</span>
                                </td>
                                <td align='right' style='width: 230px;'>
                                    <span class='Label'>Модел:&nbsp;</span>
                                </td>
                                <td align='left' style='width: 185px;'>
                                    <span class='ValueLabel'>" + tractorModelName + @"</span>
                                </td>
                             </tr>
                              <tr>
                                <td align='right' style='width: 165px;'>
            ...
            (!isTractorKindHidden ? "<th style='width: 120px;'>Вид</th>" : "") +
            (!isTractorMakeHidden ? "<th style='width: 120px;'>Марка</th>" : "") +
            (!isTractorModelHidden ? "<th style='width: 120px;'>Модел</th>" : "") +
            ...
            foreach (ReportTractorManageBlock reportTractorManageBlock in reportTractorManageBlocks)
            {
                html.Append(@"
                          <tr>
                            <td align='center'>" + counter + @"</td>" +
            (!isNormativeTechnicsHidden ? "<td align='left'>" + reportTractorManageBlock.NormativeTechnicsCode + @"</td>" : "") +
            (!isRegNumberHidden ? "<td align='left'>" + reportTractorManageBlock.RegNumber + @"</td>" : "") +
            (!isInventoryNumberHidden ? "<td align='left'>" + reportTractorManageBlock.InventoryNumber + @"</td>" : "") +
            (!isTechnicsCategoryHidden ? "<td align='left'>" + reportTractorManageBlock.TechnicsCategory + @"</td>" : "") +
            (!isTractorKindHidden ? "<td align='left'>" + reportTractorManageBlock.TractorKind + @"</td>" : "") +
            (!isTractorMakeHidden ? "<td align='left'>" + reportTractorManageBlock.TractorMake + @"</td>" : "") +
            (!isTractorModelHidden ? "<td align='left'>" + reportTractorManageBlock.TractorModel + @"</td>" : "") +
            (!isMilDepartmentHidden ? "<td align='left'>" + reportTractorManageBlock.MilitaryDepartment + @"</td>" : "") +
            (!isMilRepStatusHidden ? "<td align='left'>" + reportTractorManageBlock.MilitaryReportStatus + @"</td>" : "") +
            (!isOwnershipHidden ? "<td align='left'>" + reportTractorManageBlock.Ownership + @"</td>" : "") +
            (!isAddressHidden ? "<td align='left'>" + reportTractorManageBlock.Address + @"</td>" : "") + @"
                          </tr>");
                counter++;
            }
        ...
        protected void btnGenerateExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AppendHeader("content-disposition", "attachment; filename=ReportAllTractors.xls");
        ...
        private string GenerateAllRecordsForExport()
        {
            ...
            if (!isTechnicsCategoryHidden)
                visibleColumnsCount++;
            if (!isTractorKindHidden)
                visibleColumnsCount++;
            if (!isTractorMakeHidden)
                visibleColumnsCount++;
            if (!isTractorModelHidden)
                visibleColumnsCount++;
                            ...
                            <body>
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td align='left' colspan='2'>
                                            <span style='font-weight: bold;'>" + (tractorKind != null ? tractorKind.TableValue : All) + @"</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td align='right' colspan='" + headerLeftColspan + @"'>
                                        ...
                                        <td align='left' colspan='3'>
                                            <span style='font-weight: bold;'>" + tractorMakeName + @"</span>
                                        </td>
                                        <td align='right' colspan='1'>
                                            <span style='font-weight: normal;'>Модел:&nbsp;</span>
                                        </td>
                                        <td align='left' colspan='2'>
                                            <span style='font-weight: bold;'>" + tractorModelName + @"</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td align='right' colspan='" + headerLeftColspan + @"'>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                (!isTractorKindHidden ? "<th style='width: 150px; border-bottom: 1px solid white; border-left: 1px solid white; background-color: black; color: #FFFFFF;'>Вид</th>" : "") +
                                (!isTractorMakeHidden ? "<th style='width: 150px; border-bottom: 1px solid white; border-left: 1px solid white; background-color: black; color: #FFFFFF;'>Марка</th>" : "") +
                                (!isTractorModelHidden ? "<th style='width: 150px; border-bottom: 1px solid white; border-left: 1px solid white; background-color: black; color: #FFFFFF;'>Модел</th>" : "") +
                ...
                foreach (ReportTractorManageBlock reportTractorManageBlock in reportTractorManageBlocks)
                {
                    html.Append(@"
                            <tr>
                                ...
                                (!isNormativeTechnicsHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.NormativeTechnicsCode + "</td>") : "") + 
                                (!isRegNumberHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.RegNumber + @"</td>") : "") + 
                                (!isInventoryNumberHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.InventoryNumber + @"</td>") : "") + 
                                (!isTechnicsCategoryHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.TechnicsCategory + @"</td>") : "") +
                                (!isTractorKindHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.TractorKind + @"</td>") : "") +
                                (!isTractorMakeHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.TractorMake + @"</td>") : "") +
                                (!isTractorModelHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.TractorModel + @"</td>") : "") + 
                                (!isMilDepartmentHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.MilitaryDepartment + "</td>") : "") +
                                (!isMilRepStatusHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.MilitaryReportStatus + "</td>") : "") +
                                (!isOwnershipHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.Ownership + "</td>") : "") +
                                (!isAddressHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportTractorManageBlock.Address + "</td>") : "") + @"
                              </tr>");
                    counter++;
                }
