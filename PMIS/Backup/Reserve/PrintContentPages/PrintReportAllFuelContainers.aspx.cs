...
{
    public partial class PrintReportAllFuelContainers : RESPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                this.GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS") != UIAccessLevel.Hidden)
            {
                ...
                {
                    fuelContainerKindId = Request.Params["FuelContainerKindId"];
                }

                if (!String.IsNullOrEmpty(Request.Params["FuelContainerTypeId"]))
                {
                    fuelContainerTypeId = Request.Params["FuelContainerTypeId"];
                }
        ...
        {
            bool isNormativeTechnicsHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_NORMATIVETECHNICS") == UIAccessLevel.Hidden;
            bool isInventoryNumberHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_INVENTORYNUMBER") == UIAccessLevel.Hidden;
            bool isTechnicsCategoryHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_TECHNICSCATEGORY") == UIAccessLevel.Hidden;
            bool isFuelContainerKindHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_FUELCONTAINERKIND") == UIAccessLevel.Hidden;
            bool isFuelContainerTypeHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_FUELCONTAINERTYPE") == UIAccessLevel.Hidden;
            bool isFuelContainerCountHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_FUELCONTAINERCOUNT") == UIAccessLevel.Hidden;
            bool isMilDepartmentHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_MILDEPARTMENT") == UIAccessLevel.Hidden;
            bool isMilRepStatusHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_MILREPSTATUS") == UIAccessLevel.Hidden;
            bool isOwnershipHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_OWNERSHIP") == UIAccessLevel.Hidden;
            bool isAddressHidden = GetUIItemAccessLevel("RES_REPORTS_FUEL_CONTAINERS_ADDRESS") == UIAccessLevel.Hidden;


            ReportFuelContainerManageFilter filter = new ReportFuelContainerManageFilter()
            {
            ...
            List<ReportFuelContainerManageBlock> reportFuelContainerManageBlocks = ReportFuelContainerUtil.GetAllReportFuelContainerManageBlocks(filter, 0, CurrentUser);
            ...
            GTableItem fuelContainerKind = null;
            int railKindId = 0;
            if (int.TryParse(fuelContainerKindId, out railKindId))
            {
                fuelContainerKind = GTableItemUtil.GetTableItem("FuelContainerKind", railKindId, ModuleUtil.RES(), CurrentUser);
            }

            GTableItem fuelContainerType = null;
            int railTypeId = 0;
            if (int.TryParse(fuelContainerTypeId, out railTypeId))
            {
                fuelContainerType = GTableItemUtil.GetTableItem("FuelContainerType", railTypeId, ModuleUtil.RES(), CurrentUser);
            }
            ...
            html.Append(@"<table style='padding: 5px; width: 1024px;'>
                             ...
                             <tr>
                                ...
                                <td align='left' style='width: 205px;'>
                                    <span class='ValueLabel'>" + (fuelContainerKind != null ? fuelContainerKind.TableValue : All) + @"</span>
                                </td>
                                <td align='right' style='width: 140px;'>
                                    <span class='Label'>Тип:&nbsp;</span>
                                </td>
                                <td align='left' style='width: 205px;'>
                                    <span class='ValueLabel'>" + (fuelContainerType != null ? fuelContainerType.TableValue : All) + @"</span>
                                </td>
                             </tr>
                             <tr>
                                <td align='right' style='width: 165px;'>
            ...
            if (reportFuelContainerManageBlocks.Count() > 0)
            {
                html.Append(@"
            ...
            (!isInventoryNumberHidden ? "<th style='width: 110px;'>Инвентарен номер</th>" : "") +
            (!isTechnicsCategoryHidden ? "<th style='width: 200px;'>Категория</th>" : "") +
            (!isFuelContainerKindHidden ? "<th style='width: 160px;'>Вид</th>" : "") +
            (!isFuelContainerTypeHidden ? "<th style='width: 160px;'>Тип</th>" : "") +
            (!isFuelContainerCountHidden ? "<th style='width: 50px;'>Брой</th>" : "") +
            ...
            foreach (ReportFuelContainerManageBlock reportFuelContainerManageBlock in reportFuelContainerManageBlocks)
            {
                html.Append(@"
                          <tr>
                            <td align='center'>" + counter + @"</td>" +
            (!isNormativeTechnicsHidden ? "<td align='left'>" + reportFuelContainerManageBlock.NormativeTechnicsCode + @"</td>" : "") +
            (!isInventoryNumberHidden ? "<td align='left'>" + reportFuelContainerManageBlock.InventoryNumber + @"</td>" : "") +
            (!isTechnicsCategoryHidden ? "<td align='left'>" + reportFuelContainerManageBlock.TechnicsCategory + @"</td>" : "") +
            (!isFuelContainerKindHidden ? "<td align='left'>" + reportFuelContainerManageBlock.FuelContainerKind + @"</td>" : "") +
            (!isFuelContainerTypeHidden ? "<td align='left'>" + reportFuelContainerManageBlock.FuelContainerType + @"</td>" : "") +
            (!isFuelContainerCountHidden ? "<td align='left'>" + reportFuelContainerManageBlock.ItemsCount + @"</td>" : "") +
            (!isMilDepartmentHidden ? "<td align='left'>" + reportFuelContainerManageBlock.MilitaryDepartment + @"</td>" : "") +
            (!isMilRepStatusHidden ? "<td align='left'>" + reportFuelContainerManageBlock.MilitaryReportStatus + @"</td>" : "") +
            (!isOwnershipHidden ? "<td align='left'>" + reportFuelContainerManageBlock.Ownership + @"</td>" : "") +
            (!isAddressHidden ? "<td align='left'>" + reportFuelContainerManageBlock.Address + @"</td>" : "") + @"
                          </tr>");
                counter++;
            }
        ...
        protected void btnGenerateExcel_Click(object sender, EventArgs e)
        {
            ...
            Response.Clear();
            Response.AppendHeader("content-disposition", "attachment; filename=ReportAllFuelContainers.xls");
        ...
        private string GenerateAllRecordsForExport()
        {
            ...
            if (!isTechnicsCategoryHidden)
                visibleColumnsCount++;
            if (!isFuelContainerKindHidden)
                visibleColumnsCount++;
            if (!isFuelContainerTypeHidden)
                visibleColumnsCount++;
            if (!isFuelContainerCountHidden)
                visibleColumnsCount++;
                            ...
                            <body>
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td align='left' colspan='2'> 
                                            <span style='font-weight: bold;'>" + (fuelContainerKind != null ? fuelContainerKind.TableValue : All) + @"</span>
                                        </td>
                                        <td align='right' colspan='1'>
                                            <span style='font-weight: normal;'>Тип:&nbsp;</span>
                                        </td>
                                        <td align='left' colspan='2'>
                                            <span style='font-weight: bold;'>" + (fuelContainerType != null ? fuelContainerType.TableValue : All) + @"</span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td align='right' colspan='" + headerLeftColspan + @"'>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                (!isFuelContainerKindHidden ? "<th style='width: 150px; border-bottom: 1px solid white; border-left: 1px solid white; background-color: black; color: #FFFFFF;'>Вид</th>" : "") +
                                (!isFuelContainerTypeHidden ? "<th style='width: 150px; border-bottom: 1px solid white; border-left: 1px solid white; background-color: black; color: #FFFFFF;'>Тип</th>" : "") +
                                (!isFuelContainerCountHidden ? "<th style='width: 150px; border-bottom: 1px solid white; border-left: 1px solid white; background-color: black; color: #FFFFFF;'>Брой</th>" : "") +
                ...
                foreach (ReportFuelContainerManageBlock reportFuelContainerManageBlock in reportFuelContainerManageBlocks)
                {
                    html.Append(@"
                            <tr>
                                ...
                                (!isNormativeTechnicsHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.NormativeTechnicsCode + "</td>") : "") + 
                                (!isInventoryNumberHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.InventoryNumber + @"</td>") : "") + 
                                (!isTechnicsCategoryHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.TechnicsCategory + @"</td>") : "") + 
                                (!isFuelContainerKindHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.FuelContainerKind + @"</td>") : "") + 
                                (!isFuelContainerTypeHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.FuelContainerType + @"</td>") : "") + 
                                (!isFuelContainerCountHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.ItemsCount + @"</td>") : "") +
                                (!isMilDepartmentHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.MilitaryDepartment + "</td>") : "") +
                                (!isMilRepStatusHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.MilitaryReportStatus + "</td>") : "") +
                                (!isOwnershipHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.Ownership + "</td>") : "") +
                                (!isAddressHidden ? ("<td align='left' style='border: 1px solid black;'>" + reportFuelContainerManageBlock.Address + "</td>") : "") + @"
                              </tr>");
                    counter++;
                }
